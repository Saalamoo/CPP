...
void BitcoinExchange::run(const string &filename)
{
    ...
    while (std::getline(file, line))
    {
        ...
        try {
            ...
            while (it == _data.end() && !errorOccurred) {
                const string& currentDate = prevDate;
                prevDate = previousDate(currentDate);
                it = _data.find(prevDate);
				if (prevDate < "2009-01-02") {
					std::cout << "Error: Reached earliest date in data" << std::endl;
					errorOccurred = true;
					break;
				}
            ...
            std::cout << date << " => " << value  << " = "
                << ft_stod(it->second) * ft_stod(value) << std::endl; // << std::fixed << std::setprecision(2) 
